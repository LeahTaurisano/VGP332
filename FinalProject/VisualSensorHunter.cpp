...
void VisualSensorHunter::Update(Agent& agent, MemoryRecords& memory, float deltaTime)
{
	...
	for (const Entity* entity : entities)
	{
		...
		if (iter != memory.end())
		{
			...
			if (entity->GetTypeId() == static_cast<uint32_t>(AgentType::Gatherer))
			{
				const Gatherer* mineral = static_cast<const Gatherer*>(entity);
	...
	if (drawDebug)
	{
		...
		X::DrawScreenLine(agent.position, agent.position + fovStart, X::Colors::Red);
		X::DrawScreenLine(agent.position, agent.position + fovEnd, X::Colors::Red);

		float angle = atan2(agent.heading.y, agent.heading.x);
		X::DrawScreenArc(agent.position, viewRange, angle - viewHalfAngle, angle + viewHalfAngle, X::Colors::Red);
	}
}

void VisualSensorHunter::SetDrawDebug(bool state)
{
	drawDebug = state;
}

bool VisualSensorHunter::GetDrawDebug()
{
	return drawDebug;
}
